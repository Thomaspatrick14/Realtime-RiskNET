...
if (not args.train) or args.test_exp:
    ...
    if platform == "win32":
        model.load_state_dict(torch.load(run_path + '/checkpoint_best_epoch.pth', map_location=torch.device('cpu')))
    else:
    ...
    det_model = torchvision.models.detection.fasterrcnn_resnet50_fpn(weights=torchvision.models.detection.FasterRCNN_ResNet50_FPN_Weights.DEFAULT)
    if torch.cuda.is_available():
