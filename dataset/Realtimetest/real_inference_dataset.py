...
class RealInferenceDataset:
    ...
    def get_masks(self):
        ...
        frame_detections = self.detections[0]
        boxes = np.array(frame_detections['BBoxes'])
        classes = np.array(frame_detections['Classes'])
        processed_boxes, processed_classes = self.filter_boxes(boxes, classes, case, THRESHOLD=threshold)
        mask = self.masks_from_boxes(self.img_size, processed_boxes, divide_box_coordinates=self.divide)
        mask = mask * prior
        run_masks[0, 0, :, :] = mask
        if self.viz:
    ...
    def filter_boxes(self, boxes, classes, case, THRESHOLD=500):  # abblation study:
    ...
    def masks_from_boxes(self, img_size, boxes, divide_box_coordinates=True):
        """
