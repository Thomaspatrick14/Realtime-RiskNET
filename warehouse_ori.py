...
def append_detections_masks(pred_model, det_model,  args, img_size, video_path):
    # Cold start the models
    print('-'*79 + "\nCold starting the models...")
    cold_frame = np.ones((360, 480, 3))
    cold_masks = get_masks(detect(cold_frame), img_size, args.mask_method, args.mask_prior)
    ...
    while True:
        ...
        #         break
        mask = get_masks(detect(frame), img_size, args.mask_method, args.mask_prior)
...
def append_detections_masks_viz(pred_model, args, img_size, video_path):
    ...
    while True:
        ...
        if frame_count == 0 or frame_count % multiple == 0:
            ...
            # if run_labels_data[frame_count] == -1: # For ablation study
            #     break
            mask, processed_boxes, dboxes = get_masks(detect(frame), img_size, args.mask_method, args.mask_prior, viz)
            dmask = mask[0,0,:,:]
